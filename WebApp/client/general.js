var requestUrl = "http://103.238.162.32:5314"; // place here the URL to the server
var stopwatch = 0;
var m_topicId = '';
...
function initializeTopic(topicId) { //, topicName) {
    if (topicId == null) {
        ...
        sendRequest({
            "clientId": clientId,
            "request_get_initial_summary": {
                "topicId": topicId,
                "summaryType": summaryType,
                "summaryWordLength": initialSummWordLen,
                "questionnaireBatchIndex": questionnaireBatchInd,
...
function SubmitToAMT(callbackSuccess) {
    ...
    document.getElementById("turkSubmit").submit();
...
function handleJsonReply(jsonObj) {
    ...
    if ('error' in jsonObj) {
        ...
        // nothing to do
    } else if ("reply_get_prompt_alias" in jsonObj) {
        // nothing to do
        alert("reply_get_prompt_alias");
        alert(JSON.stringify(jsonObj));
        document.getElementById("alias_result").innerHTML = JSON.stringify(jsonObj);
    } else {
        if (isWaitingForInitial) {
            isWaitingForInitial = false;
            setNoTopicChosen();
